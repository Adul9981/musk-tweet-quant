...
export function PositionManager({ positions, onAdd, onDelete, rangeOptions, currentMarketSlug }: Props) {
  ...
  const enriched = positions.map(pos => {
    ...
    return { ...pos, currentPrice, modelProb, currentValue, pnl, pnlPct };
  });

  // ── Merge positions with the same range ──
  const rangeOrder = rangeOptions.map(r => r.range);
  const groupMap = new Map<string, typeof enriched>();
  for (const pos of enriched) {
    const g = groupMap.get(pos.range) ?? [];
    g.push(pos);
    groupMap.set(pos.range, g);
  }
  const merged = Array.from(groupMap.entries())
    .sort((a, b) => {
      const ia = rangeOrder.indexOf(a[0]);
      const ib = rangeOrder.indexOf(b[0]);
      return (ia === -1 ? 999 : ia) - (ib === -1 ? 999 : ib);
    })
    .map(([range, group]) => {
      const totalAmount   = group.reduce((s, p) => s + p.amount, 0);
      const totalShares   = group.reduce((s, p) => s + p.shares, 0);
      const avgEntry      = totalShares > 0 ? (totalAmount / totalShares) * 100 : 0;
      const currentPrice  = group[0].currentPrice;
      const modelProb     = group[0].modelProb;
      const totalValue    = currentPrice > 0 ? totalShares * (currentPrice / 100) : 0;
      const totalPnl      = totalValue - totalAmount;
      ...
      else if (currentPrice > 0 && currentPrice <= avgEntry * 0.4) signal = 'stoploss';
      else if (modelProb > 0 && modelProb < 3) signal = 'modelexit';

      return { range, avgEntry, totalAmount, totalShares, currentPrice, modelProb, totalValue, totalPnl, totalPnlPct, signal, tradeCount, ids };
    });

  const totalInvested = merged.reduce((s, g) => s + g.totalAmount, 0);
  const totalValue    = merged.reduce((s, g) => s + g.totalValue, 0);
  const totalPnl      = totalValue - totalInvested;
  const totalPnlPct   = totalInvested > 0 ? (totalPnl / totalInvested) * 100 : 0;
  const hasSignal     = merged.some(g => g.signal);
        ...
        <div className="bg-[#162538] rounded-2xl p-6 border border-slate-800/80">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {merged.map(g => {
                  const rowBg =
                    g.signal === 'takeprofit' ? 'border-l-2 border-l-emerald-500 bg-emerald-500/5' :
                    g.signal === 'stoploss'   ? 'border-l-2 border-l-rose-500 bg-rose-500/5' :
                    g.signal === 'modelexit'  ? 'border-l-2 border-l-amber-500 bg-amber-500/5' : '';
                  return (
                    <tr key={g.range} className={`border-b border-slate-800/60 hover:bg-slate-800/30 transition-colors ${rowBg}`}>
                        ...
                        <div>
                          ...
                          {g.signal && (
                            <span className={`ml-2 text-[10px] px-1.5 py-0.5 rounded font-semibold ${
                              ...
                              {g.signal === 'takeprofit' ? '止盈↑' : g.signal === 'stoploss' ? '减仓↓' : '出场'}
                            </span>
                          )}
                          {g.tradeCount > 1 && (
                      ...
                      <td className="py-3 px-3 text-right font-mono">
                        {g.currentPrice > 0 ? (
                          <span className={g.currentPrice >= g.avgEntry ? 'text-emerald-400' : 'text-rose-400'}>
                            {g.currentPrice.toFixed(1)}%
          ...
          <div className="mt-4 p-4 bg-sky-500/5 rounded-xl border border-sky-500/10">
            ...
            <div className="space-y-2">
              {merged.map(g => {
                const winPayout = g.totalShares;
                const netGain   = winPayout - g.totalAmount;
                return (
